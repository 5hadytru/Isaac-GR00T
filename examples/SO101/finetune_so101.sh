...
# Enable command tracing only after credentials have been loaded so secrets are
# not written to the training log.
set -x

CUDA_VISIBLE_DEVICES=0 uv run python \
    gr00t/experiment/launch_finetune.py \
    --base_model_path nvidia/GR00T-N1.6-3B \
    --dataset_path  examples/SO101/so101_bench_sim_6_slowed_1.7x_phase_governed_WM_v2.1 \
